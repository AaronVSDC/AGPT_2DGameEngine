...
namespace xc
{
    ...
    {
        Papyrus::GameObject* enemy = getOwner();

        auto bullet = std::make_unique<Papyrus::GameObject>();
        ...
        if (auto* enemyAnimation = enemy->getComponent<Papyrus::AnimationComponent>())
        {
            enemyWidth = static_cast<float>(enemyAnimation->getFrameWidth());
            enemyHeight = static_cast<float>(enemyAnimation->getFrameHeight());
        }
        else if (auto* enemyTexture = enemy->getComponent<Papyrus::TextureComponent>())
        {
        ...
        bullet->addComponent(std::make_unique<Papyrus::TextureComponent>(m_bulletTexturePath)); 

        // If your enemy bullet is animated, add AnimationComponent here BEFORE collider/body.
        bullet->addComponent(std::make_unique<Papyrus::AnimationComponent>(8, 1, 8, 12)); 


        bullet->addComponent(std::make_unique<Papyrus::PhysicsBodyComponent>());

        auto boxCollider = std::make_unique<Papyrus::BoxColliderComponent>();
        bullet->addComponent(std::move(boxCollider));


        bullet->addComponent(std::make_unique<EnemyBulletComponent>(m_bulletSpeed));

        Papyrus::SceneManager::getInstance().getCurrentScene()->add(std::move(bullet)); 
    }
}
